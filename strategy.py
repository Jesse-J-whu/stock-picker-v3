...
def calc_macd(df, with_zero_filter=False):
    ...
    实现逻辑：
      - 记录每个金叉位置
      - 从金叉位置到当前，DIF >= DEA 始终成立（用 rolling min 判断）
    """
    close = df['close']
    ...
    # 构造：从最近一次金叉开始，dif_above 始终为 1
    result = pd.Series(False, index=df.index)
    jc_idx = df.index[jc]
    for idx in jc_idx:
        # 从金叉位开始到末尾的 dif_above 连续最小值
        subsequent = dif_above.loc[idx:]
        if subsequent.min() >= 1.0:
            # 从该金叉位开始，之后所有行都标记为 True
            result.loc[idx:] = True
    return result
...
def calc_kdj(df, n=9, m1=3, m2=3):
    ...
    # J 上穿 K（金叉 J-K）且同时 K 上穿 D（金叉 K-D）
...
def run_strategy():
    print("=" * 60)
    print(f"  鸭口选股 V3 - {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 60)
...
<div class="strategy-desc">
    <strong>策略逻辑：</strong>六大指标全部通过才入选。
...
<div class="footer">
    <p>鸭口选股 V3 · 六大指标 × 三周期 · 数据来源：腾讯财经</p>
